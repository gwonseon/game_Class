...
public class CameraController : MonoBehaviour
{
    ...
    [SerializeField]
    private float xMoveSpeed = 500;      // 카메라의 y축 회전속도
    [SerializeField]
    private float yMoveSpeed = 250;       // 카메라의 x축 회전속도
    private float yMinLimit = 5;           // 카메라의 x축 회전 제한 최소 값
    private float yMaxLimit = 5;           // 카메라 x축 회전 제한 최대 값
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            x += Input.GetAxis("Mouse Y") * yMoveSpeed * Time.deltaTime;
        ...
        // 거리는 최대/ 최소 거리를 설정해서 그 값을 벗어나지 않도록 한다.
    ...
    private void LateUpdate()
    {
        ...
        // 카메라의 위치 정보 갱신
        // target의 위치를 기준으로 distance만큼 떨어져서 쫒느다
